...
import ibm_boto3
from ibm_botocore.client import Config as BotocoreConfig
import os
import yaml
...
class Config:
    ...
    DB_CONFIG = {
        "database": str(Path(__file__).parent / CONFIG['database']['path'])
    }
    
    # Constants
    PROCESSED_DATA_DIR = Path("processed-data")
    THRESHOLD = CONFIG['anomaly']['threshold']
    TABLE_NAME_UNIVARIATE = CONFIG['anomaly']['tables']['univariate']
    TABLE_NAME_MULTIVARIATE = CONFIG['anomaly']['tables']['multivariate']
    MULTIVARIATE_LOADINGS = CONFIG['anomaly']['tables']['multivariate_loadings']
    SYSDIG_EVENTS_TABLE = CONFIG['anomaly']['tables']['sysdig_events']
    SYSDIG_NOTIFICATIONS = CONFIG['anomaly']['tables']['sysdig_notifications']
    UNIVARIATE_MEDIAN_VALUE = CONFIG['anomaly']['tables']['univariate_median']
...
COS_ENDPOINT = CONFIG['cos']['endpoint']
COS_API_KEY_ID = CONFIG['cos']['api_key_id']
COS_SERVICE_ID = CONFIG['cos']['service_id']
COS_BUCKET_NAME = CONFIG['cos']['bucket_name']
IAM_AUTH_ENDPOINT = CONFIG['cos']['iam_auth_endpoint']

def get_cos_resource():
    """Create and return an IBM COS resource"""
    try:
        LOGGER.info("Creating COS resource with endpoint: %s", COS_ENDPOINT)
        return ibm_boto3.resource("s3",
            ibm_api_key_id=COS_API_KEY_ID,
            ibm_service_instance_id=COS_SERVICE_ID,
            config=BotocoreConfig(signature_version="oauth"),
            endpoint_url=f"https://{COS_ENDPOINT}",
            ibm_auth_endpoint=IAM_AUTH_ENDPOINT
        )
    except Exception as e:
        LOGGER.error("Failed to create COS resource: %s", str(e))
        raise

def upload_file_to_cos(local_file_path: str, object_name: str = None):
    """
    Upload a file to IBM Cloud Object Storage
    
    Args:
        local_file_path (str): Path to the file to upload
        object_name (str): Name of the object in COS. If None, uses the basename of local_file_path
    """
    ...
    try:
        ...
        LOGGER.info(f"Uploading {local_file_path} to COS bucket {COS_BUCKET_NAME}")
        cos.Object(COS_BUCKET_NAME, object_name).upload_file(local_file_path)
        LOGGER.info(f"Successfully uploaded {object_name} to COS")
        return True
    except Exception as e:
        LOGGER.error(f"Failed to upload {object_name} to COS: {str(e)}")
        return False
...
def main():
    ...
    try:
        ...
        # After generating parquet files and db file, upload them to COS
        output_dir = "processed-data"
        
        # Upload parquet files
        parquet_files = [f for f in os.listdir(output_dir) if f.endswith('.parquet')]
        for parquet_file in parquet_files:
            local_path = os.path.join(output_dir, parquet_file)
            upload_file_to_cos(local_path)
        
        # Upload SQLite database
        db_file = os.path.join(output_dir, "anomaly_detection.db")
        if os.path.exists(db_file):
            upload_file_to_cos(db_file)
